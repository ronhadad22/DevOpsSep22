...
def list_diff(elements):
    ...
    """
    diff_list_of_nums = [None]
    for i in range(1, len(elements)):
        diff_list_of_nums.append(elements[i] - elements[i - 1])
    return diff_list_of_nums
...
def pair_match(men, women):
    ...
    for man_name, man_age in men.items():
        for woman_name, woman_age in women.items():
            age_difference = abs(man_age - woman_age)
            if age_difference < min_difference:
                min_difference = age_difference
                closest_pair = (man_name, woman_name)
