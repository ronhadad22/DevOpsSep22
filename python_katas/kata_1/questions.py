...
def list_diff(elements):
    ...
    """
    diff_List = [[None], [elements[i+1] - elements[i] for i in range(len(elements)-1)]]
...
def pair_match(men, women):
    ...
    for man in men:
        for woman in women:
            ageDiff = abs(men[man] - women[woman])
            if ageDiff < minAgeD:
                minAgeD = ageDiff
                closeMatch = (man, woman)
    return closeMatch
...
def best_student(grades):
    ...
    """
    for i in grades:
        return list(grades.keys())[list(grades.values()).index(max(grades.values()))]
...
def caesar_cipher(str_to_encrypt):
    ...
    for i in range(len(str_to_encrypt)):
        ...
        if char == " ":
            ...
            answer += chr((ord(char) + enc - 65) % 26 + 65)
        
        else:
            answer += chr((ord(char) + enc - 97) % 26 + 97)
